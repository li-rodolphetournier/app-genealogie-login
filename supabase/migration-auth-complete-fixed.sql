...
-- ============================================
-- 
-- Ce script corrige l'erreur liée aux politiques RLS
-- Il supprime d'abord toutes les politiques avant de modifier la table
-- ============================================

-- ============================================
-- ÉTAPE 0 : Nettoyer les politiques RLS existantes
-- ============================================

-- IMPORTANT : Supprimer TOUTES les politiques RLS AVANT de modifier la table
DO $$ 
BEGIN
...
-- ============================================
-- ÉTAPE 1 : Adapter la table users pour Supabase Auth
-- ============================================
...
-- Supprimer les contraintes de foreign key existantes si elles existent
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
        -- Supprimer les contraintes de foreign key
        ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_id_fkey;
        
        -- Si la colonne id existe et n'est pas déjà de type UUID, on la laisse telle quelle
        -- Car modifier le type peut causer des problèmes avec les données existantes
        -- Pour une migration propre, vider la table d'abord ou créer une nouvelle table
    END IF;
END $$;

-- Créer la table users si elle n'existe pas (sans password_hash)
-- Si elle existe déjà, on ne la modifie pas automatiquement pour éviter les erreurs
...
-- Si la table existe déjà, ajouter la référence à auth.users (seulement si la colonne id est UUID)
DO $$ 
BEGIN
    IF EXISTS (
        ...
        -- Ajouter la référence à auth.users si elle n'existe pas
        IF NOT EXISTS (
